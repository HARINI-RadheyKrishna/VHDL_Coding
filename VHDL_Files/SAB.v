...
--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  process (BuffRobTag,Rob_TopPtr)  --using buffer tag and rob pointer
    ...
    variable BufferDepth_var : array_8_6;
    begin
      for i in 0 to 7 loop
        BufferDepth_var[i] := "00000";
      end loop;

      for i in 0 to 7 loop 
        BufferDepth_var[i] := unsigned(Rob_TopPtr) - unsigned(BuffRobTag[i]);
      end loop;
      BufferDepth <= BufferDepth_var;
	end process;
   ...
   --    entry of that store in the address buffer. In this case we need to use the SBTag to identify the address buffer
   --    entry that must be flushed because the RobTag is released when the store instruction leaves the top of the ROB
   --    and enters the store buffer.
    
   -- Task4: Write the code of the flushing process:
   -- Hints: 1. Cdb_RobDepth in the sensitivity list is the Rob depth of the instruction that is currently on Cdb, so decide
   ...
   --           instruction which is still in the ROB and hence it can NOT be the desired entry.
   ...
   --********************************************************************************************************************
    ...
    process (Cdb_Flush,Cdb_RobDepth,SB_FlushSw,SB_FlushSwTag,BufferDepth,BuffSBTag,BuffTagSel)	
		...
		variable Flush_var :  std_logic_vector (8 downto 0);
    ...
    begin
      ...
      if (Cdb_Flush = '1') then 
        for i in 0 to 7 loop
          if (BufferDepth[i] < Cdb_RobDepth and BuffTagSel(i) = '0') then 
            Flush_var[i] := '1';
          end if;
        end loop;
      end if;
      ...
      for i in 0 to 7 loop  
        if (BuffSBTag(i) = SB_FlushSwTag and BuffTagSel(i) = '1' and ( SB_FlushSw ='1')) then
          Flush_var[i] := '1';
        end if;
      end loop; 
	...
	process (Clk,Resetb) 
        begin
          ...
          if ((BuffRobTag(7) = Rob_TopPtr) AND (BufValid(7)='1')) then
